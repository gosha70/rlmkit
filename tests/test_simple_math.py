...
client = MockLLMClient(["FINAL: 8 + 9 * 54 / 2 = 8 + (9 * 54 / 2) = 8 + 243 = 251"])

config = RLMConfig(execution=ExecutionConfig(max_steps=20))
rlm = RLM(client=client, config=config)

try:
    result = rlm.run(prompt="No document content needed", query="What is 8+9*54/2?")
...
try:
    result = rlm.run(prompt="No document", query="What is 8+9*54/2?")
    print(f"Success: {result.success}")
    print(f"Answer: {result.answer}")
    print(f"Steps: {result.steps}")
