...
def test_rejects_final_after_error():
    ...
    # RLM now accepts FINAL with warning (instead of rejection) for direct reasoning
    assert result.steps >= 2, f"Should take at least 2 steps (took {result.steps})"

    # Check trace for warning about FINAL after error
    warning_found = False
    for step in result.trace:
        if step.get("role") == "system" and "Warning" in step.get("content", ""):
            warning_found = True
            print(f"\n[PASS] Found warning in trace: {step['content']}")
            break

    assert warning_found, "Should have warning about FINAL after execution failure"
