...
from rlmstudio.ui.app import _inject_rlmkit_desktop_css
from rlmstudio.ui.components.navigation import render_custom_navigation
from rlmstudio.ui.components.session_summary import render_session_summary
from rlmstudio.ui.utils import format_memory
...
def render_summary_cards() -> None:
    ...
    cost_pct = (
        (cost_diff / max(direct_cost, rlm_cost) * 100) if max(direct_cost, rlm_cost) > 0 else 0
    )
    ...
    with col2:
        st.metric(
            ...
            f"Δ ${cost_diff:.4f} ({cost_pct:.0f}%) ({cost_winner} wins)"
            if cost_diff > 0
            else "Equal",
        )
    ...
    time_pct = (
        (time_diff / max(direct_time, rlm_time) * 100) if max(direct_time, rlm_time) > 0 else 0
    )
    ...
    with col3:
        st.metric(
            ...
            f"Δ {time_diff:.2f}s ({time_pct:.0f}%) ({time_winner} wins)"
            if time_diff > 0
            else "Equal",
        )
...
def render_metrics_comparison() -> None:
    ...
    data = {
        ...
        "Delta": [
            ...
            format_memory(
                abs(
                    (rlm_metrics.memory_used_mb if rlm_metrics else 0)
                    - (direct_metrics.memory_used_mb if direct_metrics else 0)
                )
            ),
            "-",
            f"{(rlm_rating if rlm_rating is not None else 0) - (direct_rating if direct_rating is not None else 0):+}"
            if (rlm_rating is not None and direct_rating is not None)
            else "-",
        ],
    }
...
def render_cost_analysis() -> None:
    ...
    with col1:
        ...
        rlm_data = {
            ...
            "Share": [
                f"{(rlm_input_cost / max(rlm_total, 0.0001) * 100):.1f}%"
                if rlm_total > 0
                else "0%",
                f"{(rlm_output_cost / max(rlm_total, 0.0001) * 100):.1f}%"
                if rlm_total > 0
                else "0%",
                "100%",
            ],
    ...
    with col2:
        ...
        direct_data = {
            ...
            "Share": [
                f"{(direct_input_cost / max(direct_total, 0.0001) * 100):.1f}%"
                if direct_total > 0
                else "0%",
                f"{(direct_output_cost / max(direct_total, 0.0001) * 100):.1f}%"
                if direct_total > 0
                else "0%",
                "100%",
            ],
...
def render_quality_metrics() -> None:
    ...
    with col1:
        ...
        deltas_data = {
            ...
            "Delta": [f"{token_delta:+,}", f"${cost_delta:+.6f}", f"{time_delta:+.3f}s"],
            "Percentage": [
                f"{token_delta:+}",
    ...
    with col2:
        ...
        reasoning = (
            ...
            if (comparison and hasattr(comparison, "reasoning"))
    ...
    fig = go.Figure(data=[go.Bar(x=["RLM", "Direct"], y=[rlm_efficiency, direct_efficiency])])

    fig.update_layout(
        title="Cost per Token (micro-USD)", yaxis_title="Cost per Token (μ$)", height=400
    )
        ...
        fig = go.Figure(
            data=[
                go.Bar(
                    x=labels,
                    y=ratings,
                    marker_color=["#3b82f6" if label == "RLM" else "#10b981" for label in labels],
...
def get_latest_message() -> MessageWrapper | None:
    ...
    for message in reversed(messages):
        ...
        if isinstance(message, dict) and message.get("comparison_metrics") is not None:
