...
from rlmstudio.application.sandbox_vars import MODE_DIRECT, MODE_RAG, MODE_RLM
...
def render_sidebar():
    ...
    _page = render_custom_navigation()
...
def _load_conversation(conv_id: str):
    ...
    st.session_state.chat_messages = [{"role": "user", "content": m.user_query} for m in messages]
...
def render_comparison(message: dict, message_index: int = None):
    ...
    if rlm_rating is not None or direct_rating is not None:
        ...
        with rating_col2:
            if direct_rating is not None:
                st.metric(
                    "Direct Rating",
                    f"{direct_rating}/10",
                    help="Your rating of the Direct response",
                )
        ...
        with rating_col3:
            if rlm_rating is not None and direct_rating is not None:
                ...
                winner = (
                    "RLM wins" if rating_diff > 0 else "Direct wins" if rating_diff < 0 else "Tie"
                )
    ...
    with col3:
        ...
        st.metric(
            "Cost Diff", f"${comparison.cost_delta_usd:.4f} ({comparison.cost_delta_percent:.1f}%)"
        )
        st.metric(
            "Time Diff",
            f"{comparison.time_delta_seconds:.2f}s ({comparison.time_delta_percent:.1f}%)",
        )
    ...
    if comparison.recommendation:
        ...
        rlm_failed = (
            not rlm_response or not rlm_response.content or rlm_response.stop_reason == "error"
        )
        direct_failed = (
            not direct_response
            or not direct_response.content
            or direct_response.stop_reason == "error"
        )
        ...
        if not rlm_failed and not direct_failed:
            ...
            st.warning(
                "⚠️ Comparison data may be incomplete. One or both executions may have failed."
            )
...
def render_multi_result_message(message: dict, index: int):
    ...
    for i, (tab, r) in enumerate(zip(tabs, results, strict=False)):
        with tab:
...
def render_chat_input():
    ...
    try:
        ...
        from rlmstudio.ui.services import LLMConfigManager
        ...
        with left:
            ...
            with c1:
                ...
                with st.popover(label="", icon="➕", help="Add files or enable features"):
                    ...
                    if uploaded_files:
                        from rlmstudio.ui.file_processor import process_file
                        ...
                        for uploaded_file in uploaded_files:
                            ...
                            if file_info.success:
                                ...
                                st.error(
                                    f"Error processing {uploaded_file.name}: {file_info.error}"
                                )
        ...
        with st.spinner("Analyzing..."):
            try:
                ...
                if st.session_state.attached_files:
                    ...
                    file_info = {
                        "filename": f"{len(st.session_state.attached_files)} files"
                        if len(st.session_state.attached_files) > 1
                        else st.session_state.attached_files[0].filename,
