...
def needs_context_inspection(query: str) -> bool:
    ...
    for pattern in self_contained_patterns:
        if re.search(pattern, query_lower):
            ...
            if any(
                word in query_lower for word in ["file", "code", "attached", "provided", "in the"]
            ):
                return True
            return False
...
def check_dangerous_code_patterns(code: str) -> str | None:
    ...
    dangerous_patterns = [
        (
            r"\bimport\s+",
            "Import statements are not allowed. Use grep/peek/select/chunk to inspect content.",
        ),
        (
        ...
        (
            r"__import__\s*\(",
            "Direct __import__ not allowed. Use grep/peek/select/chunk to inspect content.",
        ),
